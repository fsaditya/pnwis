...
import Propang from '../../components/question/propang';
...
export default function HorizontalLinearStepper() {
  ...
  const handleSubmit = () => {
    ...
    if (!result) {
      ...
      const uploadTask = storage
        .ref(`propang/${fileName[0].name}`)
        .put(fileName[0])
        .then(data => {
          data.ref.getDownloadURL().then(url => {
            db.collection('propang')
              .add({
                ...form,
                downloadUrl: url,
  ...
  function getStepContent(step) {
    switch (step) {
      ...
      case 1:
        return <Propang handleInput={handleInput} data={form} />;
      case 2:
        return (
          <CV
  ...
  return (
    <>
      <Head>
        <title>Subbidang Protokoler dan Panggung</title>
      </Head>
      {/* <Close /> */}
      <Container>
        {isDone ? (
