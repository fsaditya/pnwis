...
import Registrasi from '../../components/question/registrasi';
...
export default function HorizontalLinearStepper() {
  ...
  const handleSubmit = () => {
    ...
    if (!result) {
      ...
      const uploadTask = storage
        .ref(`registrasi/${fileName[0].name}`)
        .put(fileName[0])
        .then(data => {
          data.ref.getDownloadURL().then(url => {
            db.collection('registrasi')
              .add({
                ...form,
                downloadUrl: url,
  ...
  function getStepContent(step) {
    switch (step) {
      ...
      case 1:
        return <Registrasi handleInput={handleInput} data={form} />;
      case 2:
        return (
          <CV
