...
import Logistik from '../../components/question/logistik';
...
export default function HorizontalLinearStepper() {
  ...
  const handleSubmit = () => {
    ...
    if (!result) {
      ...
      const uploadTask = storage
        .ref(`logistik/${fileName[0].name}`)
        .put(fileName[0])
        .then(data => {
          data.ref.getDownloadURL().then(url => {
            db.collection('logistik')
              .add({
                ...form,
                downloadUrl: url,
  ...
  function getStepContent(step) {
    switch (step) {
      ...
      case 1:
        return <Logistik handleInput={handleInput} data={form} />;
      case 2:
        return (
          <CV
