...
import Sekretaris from '../../components/question/sekretaris';
...
export default function HorizontalLinearStepper() {
  ...
  const handleSubmit = () => {
    ...
    if (!result) {
      ...
      const uploadTask = storage
        .ref(`sekretariat/${fileName[0].name}`)
        .put(fileName[0])
        .then(data => {
          data.ref.getDownloadURL().then(url => {
            db.collection('sekretariat')
              .add({
                ...form,
                downloadUrl: url,
  ...
  function getStepContent(step) {
    switch (step) {
      ...
      case 1:
        return <Sekretaris handleInput={handleInput} data={form} />;
      case 2:
        return (
          <CV
